...
export default function NumberLine({
  ...
  function renderTicks() {
    ...
    // Collect values occupied by fraction markers so we can suppress their tick labels
    const markerValues = []
    if (fractionA) markerValues.push(fractionA.numerator / fractionA.denominator)
    if (fractionB && showBoth) markerValues.push(fractionB.numerator / fractionB.denominator)
    ...
    return [...allTicks]
      ...
      .map((val, i) => {
        ...
        // Don't render a tick label if a fraction marker already sits here
        const hasMarker = markerValues.some(mv => Math.abs(mv - val) < 0.001)
